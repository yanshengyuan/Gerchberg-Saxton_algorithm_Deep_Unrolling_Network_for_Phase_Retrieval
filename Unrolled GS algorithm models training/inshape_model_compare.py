...
class FresnelLayer(nn.Module):
    def __init__(self, intensity_near):
        ...
        #self.unet_update_near = UNet()
        #'''
        self.conv_update_near = nn.Sequential(
            nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0),
            nn.LeakyReLU(inplace=True),
    ...
    def forward(self, phase, intens_far):
        ...
        phase_far_upd = phase_far_upd + self.conv_update_far(phase_far_upd)
        #phase_far_upd = self.conv_update_far(phase_far_upd)
        with torch.no_grad():
            phase_far = phase_far_upd.cpu().squeeze()
        ...
        phase_near_upd = phase_near_upd + self.conv_update_near(phase_near_upd)
        #phase_near_upd = self.unet_update_near(phase_near_upd)
        
        return phase_near_upd
 
class GSNet_Fresnel(nn.Module):
    def __init__(self, intensity_near, num_layers=30, val=False):
