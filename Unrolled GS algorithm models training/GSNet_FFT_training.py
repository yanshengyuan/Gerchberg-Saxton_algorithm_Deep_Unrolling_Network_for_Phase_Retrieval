...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, device, args, phase_init)

        # evaluate on validation set
        acc1 = validate(val_loader, model, criterion, args, phase_init)
...
def train(train_loader, model, criterion, optimizer, epoch, device, args, phase_init):
...
def validate(val_loader, model, criterion, args, phase_init):
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'FFT.pth.tar')
