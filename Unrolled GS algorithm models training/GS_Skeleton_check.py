...
from utils.loader import  get_training_dataCPU,get_validation_dataCPU
from model import GSNet_FFT, GSNet_Fresnel
...
parser.add_argument('-freq', '--print_freq', default=10, type=int)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = get_training_dataCPU(traindir)
    val_dataset = get_validation_dataCPU(valdir)
    ...
    model = GSNet_Fresnel(intensity_near)
    print(model)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(val_loader, train_loader, model, criterion, epoch, device, args)


def train(val_loader, train_loader, model, criterion, epoch, device, args):
    ...
    print("Test the GS skeleton first:")
    progress_skeleton = ProgressMeter(
        len(val_loader),
        [batch_time, data_time, losses],
        prefix="Epoch: [{}]".format(epoch))
    end = time.time()
    for i, (I, Phi, name) in enumerate(val_loader):
        ...
        plt.imsave('pred_img/'+ name[0] +'.png' ,outdata, cmap='gray')
        np.save('I-Phi_pred/' + name[0], outdata)
        ...
        if i % args.print_freq == 0:
            progress_skeleton.display(i + 1)


def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'Fresnel.pth.tar')
